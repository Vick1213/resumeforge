    ...
    ILogger<AnthropicLanguageModel> logger) : ILanguageModel
{
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Carries both provider and model (e.g. <c>anthropic/claude-sonnet-5</c>), per
    /// CONTRACTS.md §8, so <see cref="CachingLanguageModel"/> never serves a cached response
    /// generated by a different provider or model.
    /// </remarks>
    ...
    /// <summary>
    /// Maps <c>usage.input_tokens</c>/<c>usage.output_tokens</c>, with
    /// <c>usage.cache_read_input_tokens</c> onto <see cref="TokenUsage.CacheHits"/> so the
    /// UI's cost readout reflects Anthropic's own server-side prompt-cache hits, per
    /// CONTRACTS.md §8 — distinct from <see cref="CachingLanguageModel"/>'s local hits.
    /// </summary>
    private static TokenUsage ParseUsage(JsonElement root)
    {
        ...
        return new TokenUsage
        {
            ...
            ModelCalls = 1,
            CacheHits = usageEl.TryGetProperty("cache_read_input_tokens", out var ch) ? ch.GetInt32() : 0,
        };
    }
