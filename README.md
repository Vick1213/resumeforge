...
this README's examples — work end to end on a fresh clone with nothing configured. When
you do want a model in the loop, the language-model port is swappable rather than tied to
one vendor: point it at DeepSeek, OpenAI, Anthropic, or a model running locally in
[LM Studio](https://lmstudio.ai) — the whole tailoring pipeline against a local model, at
zero marginal cost per run. See [Quick start](#quick-start) for how selection works.
...
No API key needed — with nothing configured, `ResumeForge:Ai:Provider` defaults to `auto`,
which falls back to the deterministic `HeuristicLanguageModel` and runs the pipeline fully
offline against the sample knowledge base in `profile/`. Point your browser at
`http://localhost:5173`, or the API's Scalar docs at `http://localhost:5217/docs`.

To put a real model in the loop, set one of these environment variables before starting
the backend, or set `ResumeForge:Ai:Provider` explicitly:

| Provider | How to select it | Notes |
| --- | --- | --- |
| DeepSeek | `DEEPSEEK_API_KEY=...` | `auto` prefers this over Anthropic when both are set |
| Anthropic | `ANTHROPIC_API_KEY=...` | |
| OpenAI | `ResumeForge:Ai:Provider=openai` + `OPENAI_API_KEY=...` | |
| [LM Studio](https://lmstudio.ai) | `ResumeForge:Ai:Provider=lmstudio` | Local server, no key, zero marginal cost per run — `auto` never selects this on its own, since it requires a server you've deliberately started |
| Anything else OpenAI-compatible (Groq, Together, Ollama, vLLM, OpenRouter, ...) | `ResumeForge:Ai:Provider=openai` + `ResumeForge:Ai:BaseUrl=...` + `ResumeForge:Ai:Model=...` | No code change needed — every preset field is individually overridable |

`ILanguageModel.ModelId` always carries both provider and model (e.g.
`lmstudio/qwen3-8b`), so switching providers never serves a cached response generated by
a different model.
